...
const CarDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (data.message === "You cannot book your own car") {
        toast.error("Owners cannot book their own car.");
      } else {
    ...
    <>
      ...
      <div className="px-6 md:px-16 lg:px-24 xl:px-32 mt-16 pb-16 min-h-[60vh] bg-gradient-to-br from-gray-50 to-blue-50 overflow-x-hidden">
        ...
        >
          <span className="flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 group-hover:bg-primary/90 group-hover:text-white transition-all duration-200">
            <img
              ...
              alt=""
              className="rotate-180 opacity-70 w-4 h-4 group-hover:opacity-100"
            />
          </span>
          Back to all cars
        </button>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-14">
          ...
          >
            <div className="relative rounded-3xl overflow-hidden shadow-lg bg-white/80 mb-6 flex items-center justify-center p-6 max-w-3xl mx-auto">
              <img
                src={car.image}
                alt=""
                className="w-full h-auto max-h-80 object-contain drop-shadow-lg car-details-image-hover"
              />
              <div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-white/80 to-transparent pointer-events-none" />
            </div>
            <motion.div
              className="space-y-8 bg-white/80 rounded-2xl shadow-lg p-8"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                  ...
                  <div
                    key={text}
                    className={`flex flex-col items-center p-4 rounded-xl shadow bg-white/80 border border-gray-100 ${color} font-semibold text-base`}
                  >
                  ...
                  {[
                    ...
                    >
                      <img
                        ...
                        className="h-4 mr-2 text-green-500"
                        alt=""
                      />
                      {item}
                    </li>
          ...
          >
            <p className="flex items-center justify-between text-3xl text-primary font-extrabold w-full">
              {currency}
              {car.pricePerDay}
            ...
            <div className="flex flex-col gap-2 w-full">
              <label htmlFor="pickup-date" className="font-semibold">
                Pickup Date
              </label>
              <input
                value={pickupDate}
                ...
                type="date"
                className="border-2 border-gray-200 px-4 py-2 rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none bg-white shadow-sm text-base font-medium"
                required
                id="pickup-date"
            ...
            <button className="w-full bg-primary hover:bg-primary-dull transition-all py-3 font-bold text-white rounded-xl shadow-lg text-lg tracking-wide">
              Book Now
            </button>
